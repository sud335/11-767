    ...
    def stream_caminference(self):
        ...
        if cap.isOpened():
            ...
            while cv2.getWindowProperty("ODML767", 0) >= 0:
                ...
                img = cv2.putText(img, situ_comm, (0, 20),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                mut3.release()
                cmt ="Emotion: {0}, Activity: {1}".format(infer,act)
                img = cv2.putText(img, cmt, (0, 40),cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 200), 2)
                mut2.release()
